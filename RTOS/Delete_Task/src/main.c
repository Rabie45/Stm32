...
int main(void) {
	...
	/*Create two Taskes*/
	xTaskCreate(LEDTask, "LED Task", 500, NULL, 2, &xTaskHandler1);
	xTaskCreate(ButtonTask, "Button Task", 500, NULL, 2, &xTaskHandler2);
...
void LEDTask(void *params) {
	while (1) {
		/*blink led untill the button pressed*/
		//rtos_delay(1000);
		vTaskDelay(200);
...
void ButtonTask(void *params) {
	while (1) {
		 ...
		 * if teh button pressed stop blinking led and delete the task form the Scheduler
		 */
		if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_SET) {
			//rtos_delay(1000);
			//Notify to the Handler of the led task
			rtos_delay(1000);
					HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
